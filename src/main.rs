...
struct AbuseReport {
    ...
    url_owner_username: Option<String>,
    url_owner_id: Option<i64>,
}

#[derive(Serialize, Deserialize)]
struct ResolveReportRequest {
    action: String, // "dismiss", "delete_url", "ban_user"
}
...
async fn submit_abuse_report(
    ...
    let url_exists: bool = db.query_row(
        "SELECT COUNT(*) FROM urls WHERE short_code = ?1",
...
async fn admin_list_reports(
    ...
    let reports: Vec<AbuseReport> = stmt.query_map([], |row| {
        Ok(AbuseReport {
            id: row.get(0)?,
            short_code: row.get(1)?,
...
async fn main() -> std::io::Result<()> {
    ...
    println!("Allow Registration: {}", config.allow_registration);
    println!("========================================");
